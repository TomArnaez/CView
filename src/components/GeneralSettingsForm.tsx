import { Center, Checkbox, ColorInput, NumberInput, Title } from "@mantine/core";
import { useAppSettingsStore } from "../stores/appSettingsStore";

export const GeneralSettingsForm = (): JSX.Element => {
  const {saturatedPixelColor, saturatedPixelThreshold, autoSaveCaptures, setSaturatedPixelColour, setdSaturatedPixelThreshold, setAutoSaveCaptures} = useAppSettingsStore((state) => ({
  ...
  return ( 
    <>
      ...
      <ColorInput
        ...
        format="rgb"
        onChange={(color: string) => {setSaturatedPixelColour(color)}}
      />
      <NumberInput
        label="Enter the threshold for considering a pixel saturated"
        defaultValue={saturatedPixelThreshold}
        onChange={(threshold: number) => {setdSaturatedPixelThreshold(threshold)}}
        min={0}
        max={16384}
      />
    </>
